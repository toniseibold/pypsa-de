...
def add_existing_steel_plants(
    ...
    retirement = snakemake.params.sector["bof_retirement_date"]
    if retirement:
        bof = bof[bof["Out"] > int(snakemake.wildcards.planning_horizons)]
    logger.info(f"Early retirement of BOF plants: {retirement}")
    ...
    n.add(
        ...
        lifetime=bof["Lifetime"] if retirement else costs.at["blast furnace-basic oxygen furnace", "lifetime"],
    )
