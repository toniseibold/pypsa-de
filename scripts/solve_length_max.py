...
def get_co2_pipelines(fn):
    ...
    n.links.loc[CO2_de, "p_nom"] = n.links.loc[
            ...
            lambda row: discretized_capacity(
                nom_opt=row["p_nom_opt"],
                nom_max=row["p_nom_max"],
                unit_size=snakemake.params.solving["options"]["post_discretization"]["link_unit_size"]["CO2 pipeline"],
                threshold=snakemake.params.solving["options"]["post_discretization"]["link_threshold"]["CO2 pipeline"],
    ...
    links_co2_de = n.links.loc[CO2_de].sort_values(ascending=False, by="p_nom_opt")

    del n

    links_co2_de.p_nom_extendable = False
...
if __name__ == "__main__":
    ...
    if snakemake.wildcards.run=="no_co2_network":
        logger.info(f"Just storing the network for {snakemake.wildcards.run} since it's not having any co2 pipelines in 2035")
        n = pypsa.Network(snakemake.input.presolve)
    ...
    de_co2 = get_co2_pipelines(snakemake.input.presolve)

    # Load network
    n = pypsa.Network(snakemake.input.network)
    planning_horizons = "2035"

    length_max = float(snakemake.wildcards.length_max)*1e6
    logger.info(f"Restricting CO2 pipline expansion to {length_max/1e6} Mt/h km")

    if "CO2 pipeline pcipmi" in n.carriers.index:
        # get pcipmic pipelines
        domestic = n.links[(n.links.carrier=="CO2 pipeline pcipmi")
                & (n.links.bus0.str.contains("DE") & n.links.bus1.str.contains("DE"))].index
        inter = n.links[(n.links.carrier=="CO2 pipeline pcipmi")
                & (n.links.bus0.str.contains("DE") & ~n.links.bus1.str.contains("DE")
                | (~n.links.bus0.str.contains("DE") & n.links.bus1.str.contains("DE")))].index
        built = (n.links.loc[domestic, "length"].mul(n.links.loc[domestic, "p_nom"]).sum() + (n.links.loc[inter, "length"].mul(n.links.loc[inter, "p_nom"]).sum())/2)
        logger.info(f"{(built/1e6).round()} Mt/h km already built")
        length_max -= built
        logger.info(f"{(length_max/1e6).round()} Mt/h km as new limit")

    # drop all German pipelines
    logger.info("Dropping all German CO2 Pipelines")
    to_drop = n.links[
        n.links.carrier.str.contains("CO2 pipeline") &
        (n.links.p_nom_extendable) &
        ((n.links.bus0.str.startswith("DE")) | (n.links.bus1.str.startswith("DE"))) &
    ...
    # select de_co2 pipelines up to limit
    length_sum = 0
    selected_de_co2 = []

    for idx, row in de_co2.iterrows():
        # check if interconnector which are multiplied by factor 0.5
        if (de_co2.loc[idx].bus0[:2] != "DE") | (de_co2.loc[idx].bus1[:2] != "DE"):
            factor = 0.5
        else: # domestic pipeline
            factor = 1.0

        Mt_km = (row["length"] * row["p_nom"])*factor
        length_sum += Mt_km

        if length_sum <= length_max:
            selected_de_co2.append(idx)
        else:
            break

    logger.info(f"Adding {len(selected_de_co2)} pipeline(s) from {len(de_co2)} candidates.")

    n.add(
        "Link",
        selected_de_co2,
        **de_co2.loc[selected_de_co2]
    )
