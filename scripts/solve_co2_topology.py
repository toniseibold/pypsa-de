...
if __name__ == "__main__":
    ...
    # add pre-determined topology
    topology_file = (
        pathlib.Path(snakemake.input.topologies_dir)
        / "topologies"
        / f"{snakemake.wildcards.topology}.csv"
    )
    ...
    n.add(
        "Link",
        topology.index,
        bus0=topology.bus0 + " co2 stored compressed",
        bus1=topology.bus1 + " co2 stored compressed",
    ...
    p_nom_opt = n.links.loc[topology.index, "p_nom_opt"]

    throughput = n.links_t.p0[topology.index].mul(n.snapshot_weightings.generators, axis=0).abs().sum()
    total_hours = n.snapshot_weightings.generators.sum()

    cap_threshold = 1e-1
    effective_cap = p_nom_opt.where(p_nom_opt > cap_threshold)

    utilization = throughput.div(total_hours).div(effective_cap)
    utilization = utilization.fillna(0)

    topology["p_nom_opt"] = p_nom_opt
    topology["throughput"] = throughput
    topology["utilization"] = utilization
    topology.to_csv(snakemake.output.topology, index=False)
