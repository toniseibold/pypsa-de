...
def delaunay_triangulation(
    ...
    bus0 = np.asarray(bus_ids.take(u).astype(str).to_numpy(), dtype="U")
    bus1 = np.asarray(bus_ids.take(v).astype(str).to_numpy(), dtype="U")

    # Keep undirected edge naming compatible with topology naming in
    # prepare_sector_network.create_network_topology.
    swap_mask = bus0 > bus1
    bus0_norm = np.asarray(np.where(swap_mask, bus1, bus0), dtype="U")
    bus1_norm = np.asarray(np.where(swap_mask, bus0, bus1), dtype="U")
    connector = np.full(bus0_norm.shape, " -> ", dtype="U4")
